...
class DirectoryApiController extends OCSController {
	...
	public function getMagicSession(string $username = '', string $token = ''): Response {
		return $this->getMagicSessionHandler->handle([
			Constants::GET_MAGIC_SESSION_REQUEST_USERNAME => $username,
			Constants::GET_MAGIC_SESSION_REQUEST_TOKEN => $token,
		]);
	}
	...
	public function meGet(string $deviceUid = '', int $timestamp = 0): Response {
		return $this->meHandler->handle([
			Constants::ME_REQUEST_DEVICE_UID => $deviceUid ?: null,
			Constants::ME_REQUEST_TIMESTAMP => $timestamp,
		]);
	}
	...
	public function mePost(string $deviceUid = '', int $timestamp = 0): Response {
		return $this->meHandler->handle([
			Constants::ME_REQUEST_DEVICE_UID => $deviceUid ?: null,
			Constants::ME_REQUEST_TIMESTAMP => $timestamp,
		]);
	...
	public function putKey(string $identity = ''): Response {
		return $this->putKeyHandler->handle([
			Constants::PUT_KEY_REQUEST_IDENTITY => $identity,
		]);
	}
	...
	public function search(string $filter = ''): Response {
		return $this->searchHandler->handle([
			Constants::SEARCH_REQUEST_FILTER => $filter,
		]);
	}
	...
	public function list(int $timestamp = 0): Response {
		return $this->listUsersHandler->handle([
			Constants::LIST_USERS_REQUEST_TIMESTAMP => $timestamp,
		]);
	}
	...
	public function groups(?int $timestamp = null): Response {
		return $this->groupsHandler->handle([
			Constants::GROUPS_REQUEST_TIMESTAMP => $timestamp,
		]);
	}
