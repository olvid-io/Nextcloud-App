...
namespace OCA\OIDCIdentityProvider\Db {
	if (!class_exists(\OCA\OIDCIdentityProvider\Db\ClientMapper::class)) {
		class ClientMapper {
			public function getByIdentifier(string $identifier): ?Client { return null; }
			public function insert(object $client): object { return $client; }
		}
	}
	...
	if (!class_exists(\OCA\OIDCIdentityProvider\Db\Client::class)) {
		class Client {
			...
			public function getClientIdentifier(): string { return 'test-client-id'; }
			public function getSecret(): string { return 'test-client-secret'; }
		}
	}
}
...
namespace OCA\OIDCIdentityProvider\Event {
	if (!class_exists(\OCA\OIDCIdentityProvider\Event\TokenValidationRequestEvent::class)) {
		class TokenValidationRequestEvent {
			...
			public function getIsValid(): bool { return $this->valid; }
			public function setIsValid(bool $valid): void { $this->valid = $valid; }
